...
| 工作流              | 文件                 | 触发方式      | 功能描述                       |
| ------------------- | -------------------- | ------------- | ------------------------------ |
| 🐳 Build and Push   | `build-and-push.yml` | 手动触发      | 构建 Docker 镜像并推送到 GHCR  |
| 🚀 Deploy to Server | `deploy.yml`         | 手动触发      | 部署到 staging/production 环境 |
| 🔄 CI/CD Pipeline   | `ci-cd.yml`          | 代码推送/PR   | 自动化构建、测试、部署         |
| 🧪 Test Suite       | `test.yml`           | 代码推送/PR   | 运行各种测试                   |
| 🚀 Release Version  | `release.yml`        | 标签推送/手动 | 创建发布版本                   |

### 🎯 工作流特性

#### 1. Build and Push 工作流 (`build-and-push.yml`)
...
#### 2. Deploy to Server 工作流 (`deploy.yml`)
