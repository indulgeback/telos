...
type SidebarNavProps = {
  ...
  showFooter?: boolean
  /** Choose which sidebar content remains visible while collapsed. */
  collapsedMode?: 'navigation' | 'actions-only' | 'toggle-only'
}
...
export default function SidebarNav({
  ...
  const showWorkspace = !collapsed || collapsedMode === 'navigation'
  const showActions = !collapsed || collapsedMode !== 'toggle-only'
  const showChats = !collapsed || collapsedMode === 'navigation'
    ...
    >
      <div className='flex min-h-0 w-[224px] shrink-0 flex-col'>
        <div className='relative mb-2.5 h-10 shrink-0'>
          {showWorkspace &&
            (workspaceControl ? (
              <div className='sidebar-workspace-control absolute left-2 top-1 flex h-8 w-[164px] items-center'>
                {workspaceControl}
              </div>
            ) : (
              <button
          ...
          {showWorkspace && !workspaceControl && workspaceOpen && (
            <WorkspaceMenu
              ...
              {visibleRecents.map(item => {
                ...
                return (
                  ...
                  >
                    ...
                    >
                      <span
                        className={`sidebar-copy block min-w-0 truncate text-[14px] font-medium ${active ? 'text-ink' : 'text-ink-2'}`}
                      >
                        {item.label}
                      </span>
                    </button>
                    {item.actions && (
                      <span className='sidebar-copy shrink-0'>
                        {item.actions}
                      </span>
                    )}
                  </div>
