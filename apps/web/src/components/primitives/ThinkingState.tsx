...
export type ThinkingStateLiveData = {
  rows: Row[]
  working: boolean
  activeLabel: string
  doneLabel: string
...
export default function ThinkingState({
  ...
  variant?: string
  onSettled?: () => void
  live?: ThinkingStateLiveData
}) {
  const stage = useSequence(STAGES)
  const [manualExpanded, setManualExpanded] = useState<boolean | null>(null)
  const [selectedTool, setSelectedTool] = useState<string | null>(null)
  const v = live
    ? { active: live.activeLabel, done: live.doneLabel, rows: live.rows }
    : (VARIANTS[variant] ?? VARIANTS.Steps)
  const autoExpanded = live ? live.working : stage >= 1 && stage < 4
  const expanded = manualExpanded ?? autoExpanded
  const working = live?.working ?? stage < 3
  const visible = live
    ? v.rows.length
    : stage < 2
      ? 0
  ...
  const liveContentSignature = live?.rows
    .map(row => `${row.id ?? ''}:${row.primary}`)
    .join('\n')
  useLayoutEffect(() => {
    if (traceRef.current) setLineHeight(traceRef.current.offsetHeight)
  }, [visible, expanded, variant, stage, liveContentSignature])
  ...
  return (
    <div
      data-thinking-state={live ? 'live' : 'demo'}
      className={`flex w-full flex-col ${live ? 'max-w-none' : 'max-w-95'}`}
      style={{
              ...
              {v.rows.slice(0, visible).map((row, i) => {
                const content = (
                  <>
                    ...
                    <span
                      className={`min-w-0 text-[12.5px] ${variant === 'Reasoning' ? 'whitespace-pre-wrap break-words leading-relaxed text-ink-2' : 'truncate font-medium text-ink'} ${variant === 'Search' ? 'animated-underline' : ''}`}
                    >
                      {row.primary}
                    </span>
                    {row.secondary && (
                ...
                if (variant === 'Search') {
                  return (
                    <a
                      key={rowKey}
                      data-thinking-row={live ? rowKey : undefined}
                      href={row.href}
                      target='_blank'
                      rel='noreferrer'
