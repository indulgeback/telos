/*
 * Vendored from https://www.beautifului.dev/ on 2026-08-28.
 * Beautiful UI is MIT licensed. Keep this file aligned with the upstream source.
 */
'use client'
...
export default function ToolChips({
  ...
  const total = rows.length + (showDiffs ? 1 : 0) // rows, then diff chips
  ...
  return (
    <div
      data-tool-chips={live ? 'live' : 'demo'}
      className={`w-full pb-1 ${live ? 'min-h-0 max-w-none' : 'min-h-[220px] max-w-80'}`}
    >
      ...
      <button
        ...
        className={`${live ? 'mx-0' : '-mx-1.5'} flex w-fit items-center gap-1.5 rounded-control px-1.5 py-1 text-[12.5px] text-ink-2 transition-colors duration-100 hover:bg-hover-2`}
      >
        <svg
          width='12'
          height='12'
      ...
      >
        ...
        <div className={`${live ? 'overflow-hidden' : '-mx-1 px-1.5'} pb-1`}>
          <div className='mt-1.5 flex flex-col gap-1'>
            {(animate ? rows.slice(0, step) : rows).map((row, rowIndex) => {
              const rowKey = row.id ?? `${row.label}-${rowIndex}`
              const rowOpen = openRows.has(rowKey)
              return (
                <div
                  key={rowKey}
                  data-tool-row={live ? rowKey : undefined}
                  style={{
                    animation: live
                      ? undefined
                ...
                >
                  <button
                    ...
                    className={`group/row flex min-w-0 gap-2 rounded-control px-[3px] text-left transition-colors duration-100 hover:bg-hover-2 ${live ? 'min-h-8 w-full items-start py-1' : '-mx-[3px] h-7 w-[calc(100%+6px)] items-center'}`}
                  >
                    ...
                    <span
                      className={`inline-flex min-w-0 flex-1 cursor-pointer rounded-chip bg-field px-1.5
                    text-[11.5px] text-ink-2 shadow-hairline transition-colors duration-100 hover:bg-hover-2
                    ${live ? 'min-h-5.5 items-start whitespace-pre-wrap break-words py-0.5 leading-[1.5]' : 'h-5.5 items-center truncate'}
                    ${row.mono ? 'font-mono' : ''}`}
                    >
                      {row.chip}
                    </span>
                    ...
                    <div className='min-h-0 overflow-hidden'>
                      <div className='mt-0.5 mb-1 ml-2 flex flex-col gap-0.5 border-l border-line py-0.5 pl-3.5'>
                        {row.detail.map((line, lineIndex) => (
                          <span
                            key={`${rowKey}-detail-${lineIndex}`}
                            className={`${live ? 'whitespace-pre-wrap break-words' : 'truncate'} text-[11.5px] leading-[1.6] ${row.detailMono ? 'font-mono' : ''} ${line.tone === 'add' ? 'text-green' : 'text-ink-2'}`}
                          >
                            {line.text}
                          </span>
                        ))}
