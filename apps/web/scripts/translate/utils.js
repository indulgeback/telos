 ...
 * @param {number} delay
 */
async function executeIncrementalTranslation(
  translateFunction,
  translatorName,
  delay = 0
) {
      ...
      for (let attempt = 1; attempt <= 3; attempt++) {
        try {
          if (attempt === 1) {
            console.log(
              colors.cyan(
                `→ 正在翻译 [${colors.bold(key)}]：${colors.gray(needTranslateFlat[key])}`
              )
            )
          } else {
            console.log(
              colors.yellow(
                `重试第${attempt}次：${colors.bold(key)} - ${colors.gray(needTranslateFlat[key])}`
              )
            )
          }
          res = await translateText(
            translateFunction,
            needTranslateFlat[key],
            Source,
            Target
          )
          translated[key] = res
          console.log(
            colors.green(
              `✓ 翻译完成：${colors.bold(needTranslateFlat[key])} → ${colors.bold(res)}`
            )
          )
          success = true
          break
          ...
          if (attempt < 3) {
            console.error(
              colors.red(
                `✗ 翻译失败：${colors.bold(key)} - ${colors.gray(needTranslateFlat[key])}`
              )
            )
